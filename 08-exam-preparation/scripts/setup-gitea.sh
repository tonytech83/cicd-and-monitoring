...
# echo "* Prepare a repository (application) ..."
# sudo -u vagrant -- cp -Rv /vagrant/apps/task-manager-app /tmp/task-manager-app
...
# sudo -u vagrant -- git push -o repo.private=false --set-upstream http://vagrant:vagrant@192.168.56.12:3000/vagrant/task-manager-app main

# echo "* Prepare a repository (infrastructure) ..."
# sudo -u vagrant -- cp -Rv /vagrant/apps/task-manager-infra /tmp/task-manager-infra
# cd /tmp/task-manager-infra
# sudo -u vagrant -- git init
# sudo -u vagrant -- git checkout -b main
# sudo -u vagrant -- git add . 
# sudo -u vagrant -- git commit -m "first commit"
# sudo -u vagrant -- git push -o repo.private=false --set-upstream http://vagrant:vagrant@192.168.56.12:3000/vagrant/task-manager-infra main

# We do not need this for M6
...
# echo "* Add SOPS_AGE_KEY secret ..."
# curl -X 'PUT' -v 'http://192.168.56.12:3000/api/v1/repos/vagrant/task-manager/actions/secrets/SOPS_AGE_KEY' \
#   -H 'accept: application/json' \
#   -H 'authorization: Basic '$(echo -n 'vagrant:vagrant' | base64) \
#   -H 'Content-Type: application/json' \
#   -d '{ "data": "'$(grep SECRET /home/vagrant/.config/sops/age/keys.txt)'" }'
