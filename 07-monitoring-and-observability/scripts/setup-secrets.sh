...
sed -i "s/<KEY>/$(grep 'public key' /home/vagrant/.config/sops/age/keys.txt | cut -d ':' -f 2 | tr -d ' ')/g" /vagrant/apps/task-manager-infra/.sops.yaml 

echo "* Encrypt the secrets.yaml file ..."
sudo -u vagrant -- helm secrets encrypt /vagrant/apps/task-manager-infra/charts/task-manager/secrets.yaml > /vagrant/apps/task-manager-infra/charts/task-manager/secrets.enc.yaml
