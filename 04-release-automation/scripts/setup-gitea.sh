...
git config --global user.email vagrant@do2.lab

echo "* Prepare a repository ..."
cp -Rv /vagrant/apps/task-manager-base /tmp/task-manager
cd /tmp/task-manager && \
git init && \
git checkout -b main && \
git add . && \
git commit -m "first commit" && \
git push -o repo.private=false --set-upstream http://vagrant:vagrant@192.168.56.12:3000/vagrant/task-manager main
